$(document).ready(function () {
    var _apiBaseUrl='http://massivemoose-api.azurewebsites.net';
    updateWall();
    ...
    $('[data-action=upload]').click(function (e) {
        ...
        $.post(_apiBaseUrl+'/literally/receive/' + _brickInUse.sessionToken,
            ...
            .done(function() {
                ...
                if (brickView) {
                    ...
                    brickView.append('<img src="'+_apiBaseUrl+'/v1/image/t/' + _brickInUse.AddressX + '/' + _brickInUse.AddressY);
                }
    ...
    function updateWall() {
        $('#drawSpace').hide();
        $('#save-etc').hide();
        $.getJSON(_apiBaseUrl+'/v2/wall/0/0',null,
            function (data) {
                _wall = data;
                $('#wall').html('');
                for (var y = 0; y < data.length; y++) {
                    ...
                    for (var x = 0; x < data[y].length; x++) {
                        ...
                        if (brick && brick.Id != 0) {
                            brickView.append('<img src="'+_apiBaseUrl+'/v1/image/t/' + brick.AddressX + '/' + brick.AddressY + '" />');
                        }
    ...
    function openSession(x, y,addressX,addressY) {
        $.post(_apiBaseUrl+'/v1/image/begin/' + addressX + '/' + addressY)
            .done(function (data) {
